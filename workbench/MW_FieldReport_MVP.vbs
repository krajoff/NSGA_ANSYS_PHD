...
Select Case ubound(BoundaryArray)
  Case 9
    SolutionType = 2
	GenLength = (Ksr*(LengthCore-Bs*Ns)+LengthDamper)/2+2*(AirGap+(AirGapMax-AirGap)/3)  
  Case 7
  	SolutionType = 1	
	GenLength = (Ksr*(LengthCore-Bs*Ns)+PoleLength)/2+2*(AirGap+(AirGapMax-AirGap)/3) 
  Case 5
  	SolutionType = 0
	GenLength = (Ksr*(LengthCore-Bs*Ns)+PoleLength)/2+2*(AirGap+(AirGapMax-AirGap)/3) 
End Select
...
s = s & "RESULTS" & " (CalArea=" & CStr(CalArea) & ", " & "SolutionType=" & CStr(SolutionType) & ", GenLength=" & CStr(round(GenLength,0)) & "mm)" & Chr(13) & Chr(10)
...
' s = s & "B" & CStr(i) & "." & topvalue(0) & Chr(13) & Chr(10)
