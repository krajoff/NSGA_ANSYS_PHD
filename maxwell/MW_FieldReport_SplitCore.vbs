...
' -----------------------------------------------
' Return stator core area/volume. Updated March 2, 2021  
' -----------------------------------------------
StatorCoreVolume = 0
oModule.CalcStack "clear"
For i = 0 to Nsuns-1 
  oModule.EnterScalar 1
  oModule.EnterVol "DiaYoke_" & CStr(i)
  ...
  StatorCoreVolume = StatorCoreVolume + CDbl(TopValue(0))
Next  
StatorCoreVolume = StatorCoreVolume*LengthCore*10^(-3)
oModule.CalcStack "clear"

' -----------------------------------------------  
' Return stator core losses. Updated March 2, 2021 
' -----------------------------------------------
StatorCoreLosses = 0
For i = 0 to Nsuns-1
  If Solver = 0 then
  ...
  oModule.ClcEval "Setup : Transient", Array("Time:=", CStr(1.0*10^9/(Frequency*RtFrq)) & "ns")
  TopValue = oModule.GetTopEntryValue("Setup : Transient", Array("Time:=", CStr(1.0*10^9/(Frequency*RtFrq)) & "ns"))
  StatorCoreLosses = StatorCoreLosses + CDbl(TopValue(0))
Next
oModule.CalcStack "clear"
If Solver = 0 then
	oModule.CopyNamedExprToStack "coreLoss"
Else 
	oModule.CopyNamedExprToStack "Core_Loss"
End If
oModule.EnterVol "DiaYoke_1_Slot"
oModule.CalcOp "Integrate"
oModule.ClcEval "Setup : Transient", Array("Time:=", CStr(1.0*10^9/(Frequency*RtFrq)) & "ns")
TopValue = oModule.GetTopEntryValue("Setup : Transient", Array("Time:=", CStr(1.0*10^9/(Frequency*RtFrq)) & "ns"))
StatorCoreLosses = StatorCoreLosses + CDbl(TopValue(0))
oModule.CalcStack "clear" 
StatorCoreLosses = (Poles/CalArea)*(LengthCore-Ns*Bs)*10^(-3)*Ksr*StatorCoreLosses
' -----------------------------------------------
If Solver = 0 then
	oModule.CopyNamedExprToStack "coreLoss"
Else 
...
' -----------------------------------------------
StatorCoreYoke = StatorCoreLosses - StatorCoreTooth
...
' -----------------------------------------------  
' Return stator teeth area [m^2]. Updated March 2, 2021. Accuracy is about 95 %
' -----------------------------------------------
oModule.CalcStack "clear"
oModule.EnterScalar 1
...
oModule.DeleteTraces Array("StatorVoltage:=", Array("$Poles/$CalArea*(InducedVoltage(WindingStatorAX)-InducedVoltage(WindingStatorBY))"))
oModule.DeleteTraces Array("StatorVoltage:=", Array("$Poles/$CalArea*(InducedVoltage(WindingStatorBY)-InducedVoltage(WindingStatorCZ))")) 
oModule.DeleteTraces Array("StatorVoltage:=", Array("$Poles/$CalArea*(InducedVoltage(WindingStatorCZ)-InducedVoltage(WindingStatorAX))")) 
...
For i = 1 to 2
	...
	End If
Next
  pLosses = (Poles/CalArea)*pLosses*(PoleLength/GenLength)
...
For i = 1 to 2
	...
	End If
Next
  dLosses = (Poles/CalArea)*dLosses*(LengthDamper/GenLength)
