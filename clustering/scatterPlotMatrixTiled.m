function scatterPlotMatrixTiled(X, var_labels, NumClr, IntData, var_types, need_save, RR, PValue)
% Scatter Plot Matrix
...
b = panel();
b.pack(p, p);
b.de.margin = [1.5 2.5 1.5 2.5];
b.margin = [10 10 10 10];
b.fontsize = 7;
clr = {[0 0.45 0.74], [0.85 0.33 0.1], [0.93 0.69 0.13], ...
       [0.49 0.18 0.56], [0.47 0.67 0.19], [0.6350 0.0780 0.1840]}; 
cclr = clr{NumClr};   
for i = 1:p
    for j = 1:i
        b(i, j).select();
...
%                set(h, 'FaceColor', cclr, 'EdgeColor', [0, 0, 0])
                h = histfit(X(:, i), 15, 'kernel');
...
for i = 1:p
    laxis = [];
    for j = i:p
        laxis = [laxis b(j, i).select()];
    end
    linkaxes(laxis, 'x');
    x_tick = get(b(p, i).select(), 'Xtick');
    adj = round(mean(diff(x_tick))) / 100;
    set(b(p, i).select(), 'xlim', [min(X(:, i))-adj, max(X(:, i))+adj]);
end
...
for i = 1:p
    for j = (i+1):p
        ...
        if p_value < 0.01 
             text(0.5, 0.5, sprintf('\\rho=%.2f', r), 'Units', 'normalized',...
            'HorizontalAlignment', 'center')    
        else
            text(0.5, 0.5, sprintf('\\rho=%.2f\np=%.2f', r, p_value), 'Units', 'normalized',...
            'HorizontalAlignment', 'center')
        end
        ...
        rectangle('Position',pos, 'Curvature',[1 1], 'FaceColor', cmap(ind, :), 'EdgeColor', [0.8 0.8 0.8])
        axis equal
        axis off 
        set(gca, 'xtick', [])
        set(gca, 'ytick', [])     
...
%figure('position', [10, 10, 400, 400], 'Name', ['Correlation Heatmap Cluster ' int2str(i)])
%heatmap(RR{1}, 'Colormap', parula, 'FontSize', 12);
%caxis([-1 1])
%colormap(parula); 
%grid off
%set(gca,'fontsize',16)
%saveas(gcf, [IntData.hpp '_HeatMap_Cluster=' int2str(NumClr) '.png'])


if need_save
    saveas(gcf, [IntData.hpp '_Spearman_Cluster=' int2str(NumClr) '.png'])
end
%}
end